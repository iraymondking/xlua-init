...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<HTTP_TYPE>(translator.PushHTTP_TYPE, translator.Get, translator.UpdateHTTP_TYPE);
				translator.RegisterPushAndGetAndUpdate<LoggerHelper.LOG_TYPE>(translator.PushLoggerHelperLOG_TYPE, translator.Get, translator.UpdateLoggerHelperLOG_TYPE);
        ...
        int HTTP_TYPE_TypeID = -1;
		int HTTP_TYPE_EnumRef = -1;
        
        public void PushHTTP_TYPE(RealStatePtr L, HTTP_TYPE val)
        {
            if (HTTP_TYPE_TypeID == -1)
            {
			    bool is_first;
                HTTP_TYPE_TypeID = getTypeId(L, typeof(HTTP_TYPE), out is_first);
				
				if (HTTP_TYPE_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(HTTP_TYPE));
				    HTTP_TYPE_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, HTTP_TYPE_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, HTTP_TYPE_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for HTTP_TYPE ,value="+val);
            }
			
			LuaAPI.lua_getref(L, HTTP_TYPE_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out HTTP_TYPE val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != HTTP_TYPE_TypeID)
				{
				    throw new Exception("invalid userdata for HTTP_TYPE");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for HTTP_TYPE");
                }
				val = (HTTP_TYPE)e;
            ...
            {
                val = (HTTP_TYPE)objectCasters.GetCaster(typeof(HTTP_TYPE))(L, index, null);
            }
        }
		
        public void UpdateHTTP_TYPE(RealStatePtr L, int index, HTTP_TYPE val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for HTTP_TYPE ,value="+val);
                }
            }
        ...
        int LoggerHelperLOG_TYPE_TypeID = -1;
		int LoggerHelperLOG_TYPE_EnumRef = -1;
        
        public void PushLoggerHelperLOG_TYPE(RealStatePtr L, LoggerHelper.LOG_TYPE val)
        {
            if (LoggerHelperLOG_TYPE_TypeID == -1)
            {
			    bool is_first;
                LoggerHelperLOG_TYPE_TypeID = getTypeId(L, typeof(LoggerHelper.LOG_TYPE), out is_first);
				
				if (LoggerHelperLOG_TYPE_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(LoggerHelper.LOG_TYPE));
				    LoggerHelperLOG_TYPE_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, LoggerHelperLOG_TYPE_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, LoggerHelperLOG_TYPE_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for LoggerHelper.LOG_TYPE ,value="+val);
            }
			
			LuaAPI.lua_getref(L, LoggerHelperLOG_TYPE_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out LoggerHelper.LOG_TYPE val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != LoggerHelperLOG_TYPE_TypeID)
				{
				    throw new Exception("invalid userdata for LoggerHelper.LOG_TYPE");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for LoggerHelper.LOG_TYPE");
                }
				val = (LoggerHelper.LOG_TYPE)e;
                
            }
            else
            {
                val = (LoggerHelper.LOG_TYPE)objectCasters.GetCaster(typeof(LoggerHelper.LOG_TYPE))(L, index, null);
            }
        }
		
        public void UpdateLoggerHelperLOG_TYPE(RealStatePtr L, int index, LoggerHelper.LOG_TYPE val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for LoggerHelper.LOG_TYPE ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(HTTP_TYPE[]))
			{
			    HTTP_TYPE[] array = obj as HTTP_TYPE[];
				translator.PushHTTP_TYPE(L, array[index]);
				return true;
			}
			else if (type == typeof(LoggerHelper.LOG_TYPE[]))
			{
			    LoggerHelper.LOG_TYPE[] array = obj as LoggerHelper.LOG_TYPE[];
				translator.PushLoggerHelperLOG_TYPE(L, array[index]);
				return true;
			}
            return false;
		}
